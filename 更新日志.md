...
### 变更与优化
- **全仓品牌规范对齐**：规范化全仓名称与品牌标识为 `scenara contracts`，与 `scenara`、`scenara-data`、`scenara-model` 等协同仓库命名规则保持统一，同步更新 `pyproject.toml`、`README.md` 与包元数据描述。
- **异常提示全量中文化**：全面重构 `src/scenara_contracts/repository_v1.py` 中的 Pydantic 校验异常提示以及 `scripts/repository_contracts.py`、`scripts/domain_annotation_schemas.py` 诊断输出，确保契约校验与边界错误信息的语义清晰与中文化。
- **文档体系治理与术语规范**：
  - 将根目录及文档集全面规范为中文命名（`安全策略.md`、`更新日志.md`）；
  - 更新 `docs/剩余任务计划.md`、`docs/架构说明.md`、`docs/测试说明.md` 与 `docs/景枢能力词典.md`，同步各模块基线状态、不可变发布目录及多领域输入输出基线。
- **CI 流水线与测试门禁对齐**：
  - 更新 `.github/workflows/ci.yml` 持续集成流水线，门禁检查升级为针对 `contracts/repository/v1.1.0` 执行 `--against` 兼容性门禁并生成及归档 `repository-contracts-1.2.0.zip` 构建产物。
  - 更新 `docs/剩余任务计划.md`，明确当前 9 项自动化回归测试、v1.0.0 至 v1.2.0 历史发布不可变目录以及发布包完整性验证标准。
